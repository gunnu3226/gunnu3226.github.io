var store = [{
        ...
        "excerpt":"객체 지향 프로그래밍? 컴퓨터 프로그램을 명령어의 목록으로 보는 시각에서 벗어나 여러개의 독립된 단위, 즉 객체들의 모임으로 파악하고자 하는것이다. 각각의 객체는 메시지를 주고받고, 데이터를 처리할 수 있다.(협력) 객체 지향 프로그래밍 프로그램은 유연하고 변경이 용이하게 만들기 때문에 대규모 소프트웨어 개발에 많이 사용된다. 좋은 객체 지향 설계의 5가지 원칙(SOLID) 클린코드로 유명한 로버트 마틴이...","categories": ["Study"],
        "tags": ["개발공부","Java"],
        "url": "/study/%EA%B0%9D%EC%B2%B4%EC%A7%80%ED%96%A5/",
        "teaser": null
      },{
        "title": "[TIL]2024.01.08 TIL",
        "excerpt":"오늘의 학습     프로그래머스 2문제 풀이            무인도 여행       행렬 테두리 회전하기           스프링 mvc 1편 - 백앤드 웹 개발 핵심 기술 수강            mvc프레임 워크를 직접 만들어보면서 서버에서 mvc구조가 작동하는 로직을 배웠다.          ","categories": ["TIL"],
        "tags": ["TIL","Java"],
        "url": "/til/TIL11/",
        "teaser": null
      },{
        "title": "[TIL]2024.01.09 TIL",
        "excerpt":"오늘의 학습     프로그래머스 2문제 풀이            두 큐 합 같게 만들기       큰 수 만들기           스프링 mvc 1편 - 백앤드 웹 개발 핵심 기술 수강            스프링 Annotation기반의 mvc구조를 사용해보고 전날 구현해 보았던 mvc프레임워크와 작동 원리를 비교해보았다.          ","categories": ["TIL"],
        "tags": ["TIL","Java"],
        "url": "/til/TIL12/",
        "teaser": null
      },{
        "title": "[WEB] HTTP와 HTTPS의 개념과 차이점",
        "excerpt":"HTTP란? HTTP(Hyper Text Transfer Protocol) 란 클라이언트와 서버 간 통신을 위한 프로토콜로 TCP/IP위에서 작동한다. 즉, 인터넷에서 하이퍼텍스트를 교환하기 위한 통신 규약으로, 80번 포트를 사용한다. HTTP통신은 텍스트, 이미지, 영상, JSON등 거의 모든 형태의 데이터 전송이 가능하다. HTTP는 1997년 만들어진 HTTP/1.1이 가장 보편화 되어있으며, 현재는 HTTP/2, HTTP/3까지 개발된 상태이다. 프로토콜? 컴퓨터 내부에서...","categories": ["Network"],
        "tags": ["개발공부","네트워크"],
        "url": "/network/HTTP/",
        "teaser": null
      },{
        "title": "[알고리즘] 선택 정렬(Selection Sort)",
        "excerpt":"[선택 정렬(Selection Sort)] 선택 정렬은 한 번의 배열 탐색에서 최솟값(또는 최댓값)을 찾아 정렬되지 않은 부분의 첫번째 원소와 교환하는 방식으로 정렬한다. 시간 복잡도는 최선, 평군, 최악의 경우 모두 O(n^2)을 가진다. [선택 정렬의 작동 순서] 배열에서 최솟값을 찾는다. 최솟값을 정렬되지 않은 부분의 첫번째 원소와 교환한다. 이 과정을 정렬이 완료될 때 까지(N-1번) 반복한다....","categories": ["Algorithm"],
        "tags": ["알고리즘","Java"],
        "url": "/algorithm/%EC%84%A0%ED%83%9D%EC%A0%95%EB%A0%AC/",
        "teaser": null
      },{
